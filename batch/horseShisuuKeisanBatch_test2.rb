...
class HorseShisuuKeisanBatch
  ...
  def self.execute
    ...
    debug = 0
    thisKaisaibi = "2017/01/05"

    raceInfoArray = []
    raceInfoArray.push(["c201706010104",3,1200,0])
    raceInfoArray.push(["c201706010106",3,1800,0])
    raceInfoArray.push(["c201706010107",3,2200,1])
    raceInfoArray.push(["c201706010108",4,1800,0])
    raceInfoArray.push(["c201706010110",5,1800,0])
    raceInfoArray.push(["c201706010112",4,1600,1])

    raceInfoArray.push(["c201708010103",3,1800,0])
    raceInfoArray.push(["c201708010104",3,1800,0])
    raceInfoArray.push(["c201708010107",3,1200,0])
    raceInfoArray.push(["c201708010108",4,1800,0])
    raceInfoArray.push(["c201708010109",5,1900,0])
    raceInfoArray.push(["c201708010112",4,1200,1])
...
class KaishuritsuKeisanBatch
  def self.execute
    ...
    @races = Race.find_by_sql("select * from races where kaisaibi >= '2015/01/01' and kaisaibi <= '2015/12/31' and raceRank between 3 and 9 and babashurui <> 9 order by kaisaibi")
    ...
    @races.each do |race|
      ...
      # if topFlag && secFlag && thdFlag then
        ...
        @pays = RacePay.where("racecd=:racecd",racecd:race.racecd).where("bakenKbn=:bakenKbn",bakenKbn: 1)
        @pays.each do |pay|
